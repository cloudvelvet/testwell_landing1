...
(async()=>{
 ...
 for(const [label,path,mobile] of [[content.hero.primary,links.questionBank,false],[content.participant.action,links.myTests,true],[content.header.nav.find(item=>item.link==='result').label,links.result,true]]){
  ...
  results.push({checkedAt:new Date().toISOString(),sourceHashes,requestedUrl:path,label,viewport:mobile?'390':'1280',clicks:label==='결과 확인'?2:1,finalUrl:page.url(),loginSubmitted:false});await page.close();
 }
