...
- 현재 주요 기능: 문항 작성 · 검사지 구성 · 참여자 관리 · 설문 배포 · 검사 참여 · 결과 확인
...
현재 구현은 b078712에서 가져온 Hero 네 작업(검사 만들기·응시권 입력·검사 보기·결과 확인)과 원형 기능 목록을 기준으로 한다. 위의 이전 제작 흐름/캡션 설명은 과거 구현 기록이며 현재 화면의 기능 목록으로 대체되었다. 응시권 입력의 빈 값 안내를 수정했다. 디자인·정보구조·목적지는 유지했다. 실제 응시권 사용과 인증 후 결과 확인은 이번 검사 범위에 포함하지 않는다.
